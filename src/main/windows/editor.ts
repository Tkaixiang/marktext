...
import { BrowserWindow, dialog, ipcMain, BrowserWindowConstructorOptions } from 'electron'
...
class EditorWindow extends BaseWindow {
  ...
  private _filesToOpen: Array<{ doc: any; options: any; selected: boolean }>
   ...
   */
  createWindow(rootDirectory: string | null = null, fileList: string[] = [], markdownList: string[] = [], options: any = {}) {
    ...
    const winOptions: BrowserWindowConstructorOptions = Object.assign({ x, y, width, height }, editorWinOptions, options)
    if (isLinux) {
      // @ts-ignore
    ...
    ;['maximize', 'unmaximize', 'enter-full-screen', 'leave-full-screen'].forEach((channel) => {
      ...
      win.on(channel, () => {
        // @ts-ignore
        if (win) win.webContents.send(`mt::window-${channel}`)
      })
    })
    ...
    win.on('close', (event) => {
      ...
      // @ts-ignore
      if (win) win.webContents.send('mt::ask-for-close')
   ...
   */
  openTab(filePath: string, options = {}, selected = true) {
   ...
   */
  openTabsFromPaths(filePaths: string[]) {
   ...
   */
  openTabs(fileList: Array<{ filePath: string; selected: boolean; options: any }>) {
    // TODO: Don't allow new files if quitting.
    if (this.lifecycle === WindowLifecycle.QUITTED) return
   ...
   */
  addToOpenedFiles(filePath: string) {
   ...
   */
  changeOpenedFilePath(pathname: string, oldPathname: string) {
    const { _openedFiles, browserWindow } = this
   ...
   */
  removeFromOpenedFiles(pathname: string) {
    const { _openedFiles, browserWindow } = this
   ...
   * @returns {number[]}
   */
  getCandidateScores(fileList: string[]) {
   ...
   * @param {boolean} selected Whether the tab should become the selected tab (true if not set).
   */
  _doOpenTab(rawDocument: any, options: any, selected: boolean) {
